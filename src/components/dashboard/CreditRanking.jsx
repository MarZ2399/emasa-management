...
const getMonthLabels = () => {
  ...
  const currentMonth = new Intl.DateTimeFormat('es-PE', {
    month: 'long',
  }).format(now);

  const previousDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const previousMonth = new Intl.DateTimeFormat('es-PE', {
    month: 'long',
  }).format(previousDate);
  ...
  return {
    ...
    previousMonth: capitalize(previousMonth),
  };
};
...
export default function CreditRanking({ selectedCore, codigoVendor, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await followService.getCreditRanking({
          metgrp: selectedCore.METGRP,
          codigo: codigoVendor || undefined,
        });
        const raw = res?.data ?? res;
        setData(Array.isArray(raw) ? raw : []);
      } catch (e) {
        setError(
          e.response?.data?.msgerror ||
    ...
    fetchData();
  }, [selectedCore, codigoVendor]);

  if (!selectedCore) return null;

  const { currentMonth, previousMonth } = getMonthLabels();

  const noComprados = Array.isArray(data)
    ? data.filter((c) => Number(c.ACTUAL) === 0)
    : [];

  const siComprados = Array.isArray(data)
    ? data.filter((c) => Number(c.ACTUAL) > 0)
    : [];
        ...
        <div>
          ...
          <div className="mt-1 space-y-0.5">
            ...
            <p className="text-xs text-gray-500">
              Mes anterior: <span className="font-medium text-gray-700">{previousMonth}</span> ·{' '}
              Mes actual: <span className="font-medium text-gray-700">{currentMonth}</span>
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gradient-to-r from-[#5982A6] to-[#1a2f3d]">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-white/90 uppercase tracking-wider">
                  Mes anterior ({previousMonth})
                </th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-white/90 uppercase tracking-wider">
                  Compra actual ({currentMonth})
                </th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-white/90 uppercase tracking-wider">
                  Disponible
                </th>
