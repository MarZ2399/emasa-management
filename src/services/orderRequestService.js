...
export const transmitOrder = async (idCotizac, formData) => {
  const tipoEntregaNum = tipoEntregaMap[formData.tipoEntrega] ?? 1;
  ...
  const payload = {
    ...
    // Entrega
    tipoEntrega: tipoEntregaNum, // número para FIL1PW
    observacionDespacho: formData.direccionDespacho || '',
    lugarDespacho:
      tipoEntregaNum === 3
        ? [formData.distritoNombre, formData.provinciaNombre]
            .filter(Boolean)
            .join(', ')
        : tipoEntregaNum === 2
          ? ''
          : (formData.lugarDespacho || ''),
